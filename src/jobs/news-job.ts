...
export async function runNewsJob(options: NewsJobOptions = {}): Promise<void> {
  ...
  // 常時起動モード: 外部clientを使い、接続/切断はスキップ
  // バッチモード: 自前でclientを作成・接続・切断
  const channelId = options.channelId ?? env.discordChannelId
  const notifier = createNotifier(env.discordBotToken, channelId, {
    client: options.client,
  })
  const client = notifier.getClient()
